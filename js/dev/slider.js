...
function initSliders() {
  ...
  const boxGallerySliders = document.querySelectorAll(".box-gallery__slider");
  if (boxGallerySliders.length) {
    boxGallerySliders.forEach((slider) => {
      const parentSlider = slider.parentElement.parentElement;
      const swiperNextBtn = parentSlider.querySelector(".swiper-button-next");
      const swiperPrevBtn = parentSlider.querySelector(".swiper-button-prev");
      const swiperPagination = parentSlider.querySelector(".swiper-pagination");
    ...
    teamSliders.forEach((slider) => {
      ...
      function updateSlideOpacity(swiper) {
        ...
        slides.forEach((slide2, index) => {
          slide2.classList.remove("is-visible", "is-dimmed");
        });
        if (activeIndex + slidesPerView >= totalSlides - 0.5) {
          for (let i = totalSlides - Math.ceil(slidesPerView); i < totalSlides; i++) {
            if (slides[i]) {
        ...
        slides.forEach((slide2) => {
          if (!slide2.classList.contains("is-visible")) {
            slide2.classList.add("is-dimmed");
          }
        });
      }
    });
