...
function formQuantity() {
  ...
  document.addEventListener("blur", quantityActions, true);
  function formatValue(value) {
    const intValue = parseInt(value) || 1;
    return intValue < 10 ? "0" + intValue : "" + intValue;
  }
  function updateMinusButtonState(container, value) {
    const minusButton = container.querySelector("[data-fls-quantity-minus]");
    if (parseInt(value) <= 1) {
  ...
  function quantityActions(e) {
    ...
    let currentValue = parseInt(valueElement.value) || 1;
    if (type === "click") {
      if (targetElement.closest("[data-fls-quantity-plus]")) {
        currentValue++;
        const max = +valueElement.dataset.flsQuantityMax;
        if (max && currentValue > max) currentValue = max;
      } else if (targetElement.closest("[data-fls-quantity-minus]")) {
        currentValue--;
        const min = +valueElement.dataset.flsQuantityMin || 1;
        if (currentValue < min) currentValue = min;
      }
      valueElement.value = formatValue(currentValue);
      updateMinusButtonState(container, currentValue);
    } else if (type === "input") {
      valueElement.value = valueElement.value.replace(/\D/g, "");
      updateMinusButtonState(container, valueElement.value);
    } else if (type === "blur") {
      const formatted = formatValue(valueElement.value);
      valueElement.value = formatted;
      updateMinusButtonState(container, formatted);
    }
  }
}
if (document.querySelector("[data-fls-quantity]")) {
...
(() => {
  ...
  function openMenu(menuName) {
    ...
    if (menu && button) {
      ...
      menu.style.height = `${window.innerHeight - menuBlockRect.bottom - 24}px`;
    }
  }
  function toggleMenu(menuName) {
    const menu = document.querySelector(`[data-menu-target="${menuName}"]`);
    const isOpen = menu == null ? void 0 : menu.classList.contains(activeClass);
    if (isOpen) {
      closeAllMenus();
    } else {
      openMenu(menuName);
  ...
  function setupMenuImageHover() {
    const linkMenuImg = document.querySelectorAll("[data-img-src]");
    if (linkMenuImg.length) {
      linkMenuImg.forEach((link) => {
        const parent = link.closest("[data-menu-parent]");
        if (!parent) return;
        const img = parent.querySelector("[data-menu-img]");
        if (!img) return;
        const hoverSrc = link.dataset.imgSrc;
